...
const DashboardPage: React.FC = () => {
  ...
  const fetchPosts = () => {
    if (user) {
        getJobsByUserId(user.id).then(posts => setMyPosts(posts));
    }
  };
  ...
  const handleDeleteJob = async () => {
    if (jobToDelete && user) {
      await deleteJob(jobToDelete.id, user);
  ...
  return (
    <>
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Confirm Deletion">
        <p className="text-gray-600 dark:text-gray-300">Are you sure you want to delete the job post "{jobToDelete?.title}"? This action cannot be undone.</p>
        <div className="mt-6 flex justify-end gap-4">
            <button onClick={() => setIsDeleteModalOpen(false)} className="px-4 py-2 bg-gray-200 dark:bg-slate-600 text-fog-dark dark:text-fog-light font-semibold rounded-lg hover:bg-gray-300 dark:hover:bg-slate-500 transition-colors">Cancel</button>
            <button onClick={handleDeleteJob} className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 transition-colors">Delete Job</button>
        </div>
      </Modal>
                  ...
                  <div className="flex justify-between items-center mb-4">
                   ...
                   {myPosts.length > 0 ? (
                     <ul className="space-y-3">
                         {myPosts.map(job => (
                             <li key={job.id} className="p-3 bg-gray-50 dark:bg-slate-800/50 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700/50 transition-colors flex justify-between items-center group">
                                  <div>
                                  ...
                                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => navigate(`/edit-job/${job.id}`)} className="p-2 text-gray-500 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-400 rounded-full transition-colors"><PencilIcon className="w-5 h-5" /></button>
                                    <button onClick={() => openDeleteModal(job)} className="p-2 text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400 rounded-full transition-colors"><TrashIcon className="w-5 h-5" /></button>
