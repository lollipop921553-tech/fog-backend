...
const Header: React.FC = () => {
  ...
  useClickOutside(themeDropdownRef, () => setIsThemeOpen(false));
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-3">
             <div className="relative" ref={themeDropdownRef}>
              <button onClick={() => setIsThemeOpen(!isThemeOpen)} aria-label="Toggle theme" className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-fog-mid-dark focus:outline-none focus:ring-2 focus:ring-fog-accent">
                <SunIcon className="h-5 w-5 hidden dark:block" />
                <MoonIcon className="h-5 w-5 block dark:hidden" />
              </button>
              {isThemeOpen && (
                <div className="origin-top-right absolute right-0 mt-2 w-36 rounded-md shadow-lg py-1 bg-fog-white dark:bg-fog-mid-dark ring-1 ring-black ring-opacity-5 z-50 animate-fade-in">
              ...
              {isAuthenticated && user ? (
                <>
                  <div className="relative" ref={profileDropdownRef}>
                    ...
                    {isProfileOpen && (
                      <div className="origin-top-right absolute right-0 mt-2 w-56 rounded-md shadow-lg py-1 bg-fog-white dark:bg-fog-mid-dark ring-1 ring-black dark:ring-white/10 ring-opacity-5 z-50 animate-fade-in">
