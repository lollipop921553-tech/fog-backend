...
const ChatWidget: React.FC = () => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (messages.length === 0 || messages[0].id !== 'ai-welcome') {
    ...
    return (
        <>
            <div className="fixed bottom-5 right-5 z-50">
                <button
                    onClick={toggleChat}
                    className="bg-fog-accent text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg hover:bg-fog-accent-hover transition-all transform hover:scale-110 focus:outline-none"
            ...
            {isOpen && (
                <>
                    <div className="fixed inset-0 bg-black/60 z-40 animate-fade-in" onClick={toggleChat} aria-hidden="true"></div>
                    <div 
                        className="fixed bottom-24 right-5 w-[calc(100vw-2.5rem)] sm:w-[450px] h-[calc(100vh-7.5rem)] sm:h-[70vh] max-h-[600px] bg-fog-white/80 dark:bg-fog-dark/80 backdrop-blur-xl rounded-2xl shadow-2xl dark:shadow-2xl-dark flex flex-col overflow-hidden animate-fade-in-up origin-bottom-right border border-black/10 dark:border-white/10 z-50"
                        onClick={e => e.stopPropagation()}
                        role="dialog"
                        aria-modal="true"
                        aria-labelledby="chat-widget-header"
                    >
                        <header id="chat-widget-header" className="flex items-center justify-between p-4 bg-fog-accent text-white rounded-t-xl flex-shrink-0">
                            <div className="flex items-center gap-2">
                                <AIIcon className="w-6 h-6" />
                                <h3 className="font-bold">AI Customer Service</h3>
                            </div>
                            <button onClick={toggleChat} className="p-1 rounded-full hover:bg-white/20" aria-label="Close chat">
                                <XIcon />
                            </button>
                        </header>
